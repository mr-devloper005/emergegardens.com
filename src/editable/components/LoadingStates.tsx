...
function PulseBlock({ className }: { className?: string }) {
  return <div className={cn('animate-pulse rounded-[12px] bg-white/[0.06]', className)} />
}

export function PageLoadingState({ label = 'Loading', className }: LoadingStateProps) {
  return (
    <div
      className={cn(`${dc.shell.section} py-16`, className)}
      aria-live="polite"
      aria-busy="true"
    >
      ...
      <div className="mt-10 grid gap-6 md:grid-cols-3">
        {[0, 1, 2].map((i) => (
          <div key={i} className={`${dc.surface.soft} p-5`}>
  ...
  return (
    ...
    >
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className={`${dc.surface.soft} p-4`}>
...
export function DetailLoadingState({ label = 'Loading', className }: LoadingStateProps) {
  return (
    <div
      className={cn(`${dc.shell.section} grid gap-10 py-16 lg:grid-cols-[0.8fr_1.2fr]`, className)}
      aria-live="polite"
      aria-busy="true"
    >
